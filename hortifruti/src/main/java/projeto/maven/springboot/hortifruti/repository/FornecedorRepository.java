...
public interface FornecedorRepository extends JpaRepository<FornecedorModel, Long> {
	...
	@Query(value = "select p from FornecedorModel p where upper(trim(p.nome)) like %?1%") 	// like indica que quero pegar uma parte - %?1& é porque tenho apenas um parâmetro
	List<FornecedorModel> pesquisarPorNome(String nome); 									// trim retira o espaço
